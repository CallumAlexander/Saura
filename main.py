'''
Saura GroundStation 3.3.0
Developed by Fraser Rennie & Callum Alexander
'''
...
def Record():
    ...
    while step <= period:
        ...
        if currentline1[1] == 'e':
            temp, loc[:], pressure = preprocess(currentline1, step, temp, tStamp, pressure)
        else:
            temp, loc[:], pressure = preprocess(currentline2, step, temp, tStamp, pressure)
        
        tempDiff = temp - lastTemp
        presDiff = pressure - lastPres
        ...
        if pressure > lastPres:
            print(' >>>')
        else:
            print(' <<<')
        print(' dif: ' + str(round(presDiff, 1)))
        print('------------------------------')
...
def preprocess(currentline, step, temp, tStamp, pressure):
    currentline = currentline[10:]
    #tstamp, x, y, temp, pressure
    transfer = [0,0,0,0,0]

    loc = [0,0]
    loc[0], loc[1], temp, pressure = currentline.split(",")
...
def exporter():
    ...
    worksheet.write('B2', 'Time Stamp', bold)
    worksheet.write('C2', 'x', bold)
    worksheet.write('D2', 'y', bold)
    worksheet.write('E2', 'Temperature', bold)
    worksheet.write('F2', 'Pressure', bold)
